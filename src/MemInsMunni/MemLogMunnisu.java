...
public class MemLogMunnisu extends WindowAdapter implements WindowListener {

   // 프레임설정
   // panelBorderCenter 센터
   static JFrame frameObj = new JFrame("로그인");
   ...
   // South 내역
   static JLabel labelMaxDown = new JLabel(" ", JLabel.CENTER);
   // ----------------------------------------------------------------------------------------------------------------
   ...
   // 상반
   static JLabel titleLabel = new JLabel("Login");
//   static JLabel titleApplicate = new JLabel("로그인");
   ...
   static JPanel[] emptyCenter = new JPanel[4];

   // 하반
   static JPanel lsatInnerPanel = new JPanel();
   static JButton btnApplicateDown = new JButton("로그인");
...
//      GridInner[0].add(titleApplicate);
      ...
      btnApplicateDown.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (obj.flag == 1) {
				labelMaxDown.setText(obj.errcode + "빈값입니다.");
				obj.flag = 0;
