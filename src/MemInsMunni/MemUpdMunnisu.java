...
public class MemUpdMunnisu extends WindowAdapter implements WindowListener {

	// 프레임설정
	// panelBorderCenter 센터
	static JFrame frameObj = new JFrame("회원수정");
	...
	// South 내역
	static JPanel panelBorderSouth = new JPanel(new BorderLayout());
	...
	// 상반
	static JLabel titleButton = new JLabel("회원수정");
	static JLabel titleApplicate = new JLabel();
	...
	static JButton ApplicateDown = new JButton("회원수정");
	static JButton DeApplicateDown = new JButton("초기화");
	// ----------------------------------------------------------------------------------------------------------------
	...
	public static void main(String[] args) {
		...
		MemUpdMunnisu obj = new MemUpdMunnisu();
...
//        panelArrayGrid 세팅하기.
		...
		titleButton.setIcon(new ImageIcon("./client/remove.jpg"));
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < calText.length; i++) {
					...
					if (calText[i].getText().equals("")) {
						obj.errcode = obj.errcode + " " + subTitle[i];
						obj.flag = 1;
					} else {
				...
				if (obj.flag == 1) {
					bntMaxDown.setText(obj.errcode + "빈값입니다.");
					obj.flag = 0;

					obj.errcodeArray = obj.errcode.split(" ");

					for (int i = 0; i < subTitle.length; i++) {
						if (obj.errcodeArray[1].equals(subTitle[i])) {
	...
	public static void panelBorderCenterSetting(JPanel getJPanel) {
		...
		bntMaxDown.setBackground(Color.BLUE);
